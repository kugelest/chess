...
case class Knight(getPos: String, getColor: Char, kind: String = "knight")
    extends Piece(getPos, getColor, kind) {

  override def whiteMovePossible(to: String, board: Board): Boolean = {
    if (
      getColor.equals('w') && (board.getSquare(to).get.isFree || board
        .getSquare(to)
        .get
        .mannedByBlack)
    ) {
  ...
  override def blackMovePossible(to: String, board: Board): Boolean = {
    if (
      getColor.equals('b') && (board.getSquare(to).get.isFree || board
        .getSquare(to)
        .get
        .mannedByWhite)
    ) {
  ...
  def knightMove(to: String): Boolean = {
    ...
    if (
      destFile
        .equals((origFile + 1).toChar) || destFile.equals((origFile - 1).toChar)
    ) {
      ...
      destFile
        .equals((origFile + 2).toChar) || destFile.equals((origFile - 2).toChar)
    ) {
